"""
V5A Internal Flow: GPS domain full-stack scenario. Dry-run only.
"""
...
def run(action_id="v5a_gps_001", intent="location_query"):
    action = ActionCandidate(
        action_id=action_id, domain="gps", actor_id="gps_flow",
    ...
    os3 = OS3ProofTicket(
        ticket_id=f"os3_{action_id}", action_id=action_id, domain="gps",
    ...
    ticket = issue_sovereign_ticket(
        ...
        x108_gate="ALLOW", scope="gps", autonomy_level=1,
        world_call_class=WorldCallClass.READ_ONLY_WORLD_CALL.value,
    )
    gateway = ObsidiaGateway()
    decision = gateway.check(ticket, WorldCallClass.READ_ONLY_WORLD_CALL, "gps")

    sv = GovernedStateVector()
    lyapunov = compute_lyapunov(action_id, sv)
    theta = DecisionEnvelopeTheta(theta_id="theta_gps", x108_gate="ALLOW", confidence=0.9, reason_code="ALLOW_STANDARD")
    pog = proof_of_governance(action_id, theta, lyapunov, os3)

    return {
        "action_id": action_id, "domain": "gps",
