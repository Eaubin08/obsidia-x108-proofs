...
def build_true_brody_answer(
    ...
    pre_reasoning_response_source = str(
        ctx.get("pre_reasoning_response_source") or ""
    )
    pre_reasoning_unresolved = (
        pre_reasoning_response_source
    ...
    # General conversation is a presentation regime, not semantic authority.
    # It may replace terminal/domain/memory presentation only when:
    #   - request remains PURE_RESPONSE-compatible,
    #   - semantic topic remains GENERAL,
    #   - pre-reasoning has no causal unresolved symbol.
    #
    # This preserves fail-closed behavior for e.g. "salut florvaxium".
    _general_snap_ctx = ctx.get(
    ...
    if (
        not creator_detected
        and not domain_answered
        and not pre_reasoning_unresolved
        and is_general_conversation_readonly(
            user_message,
            _general_auth_ctx,
            _general_snap_ctx,
        )
