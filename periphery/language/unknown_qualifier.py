...
# Ordinary surface-language units already used by Brody's conversational
# interface. These are not semantic knowledge entries and grant no authority.
#
# Their purpose is only to prevent ordinary wording from becoming causal
# lexical debt before C274. A genuine unknown concept remains unresolved.
...
def _semantic_vocabulary(
    ...
    fields = (
        ...
        # Only the trigger that actually produced a canonical route.
        # This does not consume unrelated lexical unknowns from the message.
        semantic_query_snapshot.get(
            "matched_trigger",
            "",
        ),
...
def qualify_unknowns(
    *,
    ...
    # Surface-language qualification is multilingual by design.
    # `language` is a response/presentation preference, not proof
    # that every token in the input belongs exclusively to that
    # language. Ordinary FR/EN function words carry no semantic
    # authority, so both supported surface vocabularies are safe
    # to recognize before C274. Genuine unknown concepts remain
