...
$ptr = Join-Path $Root "CURRENT_BRODY_API_BRIDGE_AUTHORIZATION_PACKET_READONLY_V1.txt"

if (!(Test-Path $ptr)) {
  throw "MISSING_BRODY_API_BRIDGE_AUTHORIZATION_PACKET_POINTER=$ptr"
}
...
if ($kv["STATUS"] -ne "BRODY_API_BRIDGE_AUTHORIZATION_PACKET_READONLY_V1_PASS") {
  throw "BAD_AUTHORIZATION_PACKET_STATUS=$($kv["STATUS"])"
}

if ($kv["AUTHORIZATION_PACKET"] -ne "true") { throw "AUTHORIZATION_PACKET_NOT_TRUE" }
...
Write-Host "BRODY_API_BRIDGE_AUTHORIZATION_PACKET_READONLY_V1_PASS"
