...
$ptr = Join-Path $Root "CURRENT_BRODY_API_BRIDGE_RUNTIME_ACTIVATION_GATE_READONLY_V1.txt"

if (!(Test-Path $ptr)) {
  throw "MISSING_BRODY_API_BRIDGE_RUNTIME_ACTIVATION_GATE_POINTER=$ptr"
}
...
if ($kv["STATUS"] -ne "BRODY_API_BRIDGE_RUNTIME_ACTIVATION_GATE_READONLY_V1_PASS") {
  throw "BAD_ACTIVATION_GATE_STATUS=$($kv["STATUS"])"
}

if ($kv["ACTIVATION_GATE"] -ne "true") { throw "ACTIVATION_GATE_NOT_TRUE" }
...
Write-Host "BRODY_API_BRIDGE_RUNTIME_ACTIVATION_GATE_READONLY_V1_PASS"
