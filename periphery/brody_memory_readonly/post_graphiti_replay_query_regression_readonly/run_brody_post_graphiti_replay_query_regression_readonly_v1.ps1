...
if ($OutDir -eq "") {
  ...
  $OutDir = Join-Path $workspaceRoot "_local_audits\BRODY_POST_GRAPHITI_REPLAY_QUERY_REGRESSION_READONLY_V1_$ts"
}

New-Item -ItemType Directory -Force $OutDir | Out-Null

$py = Join-Path $scriptDir "brody_post_graphiti_replay_query_regression_readonly_v1.py"
...
if ($LASTEXITCODE -ne 0) {
  throw "BRODY_POST_GRAPHITI_REPLAY_QUERY_REGRESSION_READONLY_FAILED"
}
