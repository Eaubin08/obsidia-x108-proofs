...
$checks = @(
  ...
  @{ file = "CURRENT_BRODY_OPERATOR_EXECUTION_RECEIPT_READONLY.txt"; status = "BRODY_OPERATOR_EXECUTION_RECEIPT_READONLY_V1_PASS" },
...
foreach ($check in $checks) {
  $kv = Read-Kv -Path (Join-Path $x108 $check.file)
...
$receiptClose = Read-Kv -Path (Join-Path $x108 "CURRENT_BRODY_OPERATOR_EXECUTION_RECEIPT_CLEAN_CLOSE_READONLY.txt")

powershell -NoProfile -ExecutionPolicy Bypass -File $receiptClose["RUN_PS1"] -Root $Root
if ($LASTEXITCODE -ne 0) {
  throw "RECEIPT_CLEAN_CLOSE_FAILED_DURING_OPERATOR_LINE_BASELINE"
}

python (Join-Path $x108 "proofs\verify_all.py")
if ($LASTEXITCODE -ne 0) {
  throw "VERIFY_ALL_FAILED_DURING_OPERATOR_LINE_BASELINE"
}

Write-Host "BRODY_OPERATOR_EXECUTION_LINE_BASELINE_FREEZE_READONLY_V1_PASS"
