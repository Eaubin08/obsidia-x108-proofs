...
$ptr = Join-Path $Root "CURRENT_BRODY_API_BRIDGE_DRY_RUN_READONLY_V1.txt"

if (!(Test-Path $ptr)) {
  throw "MISSING_BRODY_API_BRIDGE_DRY_RUN_POINTER=$ptr"
}
...
if ($kv["STATUS"] -ne "BRODY_API_BRIDGE_DRY_RUN_READONLY_V1_PASS") {
  throw "BAD_DRY_RUN_STATUS=$($kv["STATUS"])"
}

if ($kv["DRY_RUN"] -ne "true") { throw "DRY_RUN_NOT_TRUE" }
if ($kv["ALL_CASES_NOT_EXECUTED"] -ne "True") { throw "ALL_CASES_NOT_EXECUTED_NOT_TRUE" }
...
Write-Host "BRODY_API_BRIDGE_DRY_RUN_READONLY_V1_PASS"
