...
BOUNDARY = {
    ...
    "ui": False,
    "brody_role": "MEMORY_READONLY_MICRO_SMOKE",
...
def main():
    ...
    if not close_ptr.exists():
        raise RuntimeError(f"MISSING_CLOSE_POINTER={close_ptr}")
    ...
    if not close_summary_json.exists():
        raise RuntimeError(f"MISSING_CLOSE_SUMMARY_JSON={close_summary_json}")
    ...
    if int(close_summary.get("manual_apply_write_executed_count", 0)) != 29:
        raise RuntimeError("CLOSE_REPORT_BAD_MANUAL_APPLY_WRITE_COUNT")

    if int(close_summary.get("replay_exact_found_count", 0)) != 29:
        raise RuntimeError("CLOSE_REPORT_BAD_REPLAY_EXACT_COUNT")
    ...
    try:
        ...
        query_records = []
        prev = "GENESIS_BRODY_MEMORY_READONLY_MICRO_SMOKE_V1"
    ...
    exact_json = out_dir / "BRODY_MEMORY_READONLY_MICRO_SMOKE_EXACT_RECORDS.json"
    queries_jsonl = out_dir / "BRODY_MEMORY_READONLY_MICRO_SMOKE_QUERY_RECORDS.jsonl"
    queries_json = out_dir / "BRODY_MEMORY_READONLY_MICRO_SMOKE_QUERY_RECORDS.json"
    missing_json = out_dir / "BRODY_MEMORY_READONLY_MICRO_SMOKE_MISSING_IDS.json"
    summary_json = out_dir / "BRODY_MEMORY_READONLY_MICRO_SMOKE_SUMMARY.json"
    report_md = out_dir / "BRODY_MEMORY_READONLY_MICRO_SMOKE_REPORT.md"
    ...
    summary = {
        "status": "BRODY_MEMORY_READONLY_MICRO_SMOKE_V1_PASS" if micro_smoke_ok else "BRODY_MEMORY_READONLY_MICRO_SMOKE_V1_FAIL",
        "created_at": now_iso(),
        ...
        "latest_smoke_event_hash": prev,
        "micro_smoke_ok": micro_smoke_ok,
        "outputs": {
        ...
        "next": "BUILD_BRODY_SESSION_REOPEN_LOOP_READONLY_V1",
        **BOUNDARY,
    }
    ...
    if not micro_smoke_ok:
        raise SystemExit("BRODY_MEMORY_READONLY_MICRO_SMOKE_FAILED")
