...
$ptr = Join-Path $Root "CURRENT_BRODY_API_BRIDGE_AUTHORIZED_RUNTIME_PRECHECK_READONLY_V1.txt"

if (!(Test-Path $ptr)) {
  throw "MISSING_AUTHORIZED_RUNTIME_PRECHECK_POINTER=$ptr"
}
...
if ($kv["STATUS"] -ne "BRODY_API_BRIDGE_AUTHORIZED_RUNTIME_PRECHECK_READONLY_V1_PASS") {
  throw "BAD_PRECHECK_STATUS=$($kv["STATUS"])"
}
...
$smokePy = $kv["SMOKE_PY"]
$precheckDir = $kv["PRECHECK_DIR"]

if (!(Test-Path $smokePy)) { throw "MISSING_SMOKE_PY=$smokePy" }
if (!(Test-Path $precheckDir)) { throw "MISSING_PRECHECK_DIR=$precheckDir" }

Push-Location $precheckDir
try {
  python $smokePy
}
finally {
...
Write-Host "BRODY_API_BRIDGE_AUTHORIZED_RUNTIME_PRECHECK_READONLY_V1_PASS"
