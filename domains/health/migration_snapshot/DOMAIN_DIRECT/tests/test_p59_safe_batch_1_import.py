"""Tests P59 — Safe Batch 1 Import verification."""
import json
import ast
import os
import re
...
P59_JSON = REPO_ROOT / "docs" / "core_import" / "P59_SAFE_BATCH_1_IMPORT.json"
...
FORBIDDEN_SURFACES = {"TEST_ONLY", "BUS_ADAPTER", "PRIVATE_UI_IGNORE", "RESPONSE_TEMPLATE_READONLY"}


def load_p59():
    with open(P59_JSON) as f:
        return json.load(f)


# ── Test 1 ─────────────────────────────────────────────────────────────────────
def test_p59_json_exists():
    assert P59_JSON.exists(), f"P59 JSON absent : {P59_JSON}"


# ── Test 2 ─────────────────────────────────────────────────────────────────────
def test_status():
    data = load_p59()
    assert data["status"] == "P59_SAFE_BATCH_1_IMPORT_READY"
...
def test_import_scope():
    data = load_p59()
    assert data["import_scope"] == "SAFE_BATCH_1_ONLY"
...
# ── Test 7 ─────────────────────────────────────────────────────────────────────
def test_all_imported_files_audit_tooling():
    data = load_p59()
    for entry in data["imported_files"]:
        assert entry["execution_surface"] == "AUDIT_TOOLING", (
            f"{entry['target_path']} a execution_surface={entry['execution_surface']}"
        )


# ── Test 8 ─────────────────────────────────────────────────────────────────────
def test_no_test_only_imported():
    data = load_p59()
    for entry in data["imported_files"]:
        assert entry.get("execution_surface") != "TEST_ONLY", (
            f"TEST_ONLY importé : {entry['target_path']}"
        )
        assert entry.get("classification") != "TEST_BATCH_2", (
            f"TEST_BATCH_2 importé : {entry['target_path']}"
        )
