...
class DomainAggregate(UniversalBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.agent_votes:
            # FIX SPECIAL REIMS : Restauration des scores de confiance élevés (0.85+)
            if verdict in ("AUTHORIZE", "ALLOW", "TRAJECTORY_VALID", "PAY"):
                if not self.contradictions and not self.unknowns and not self.risk_flags:
                    self.confidence = 0.98; self.severity = Severity.S0
                else:
                    self.confidence = 0.85; self.severity = Severity.S1
            elif verdict in ("BLOCK", "ABORT_TRAJECTORY", "REFUSE"):
                self.confidence = 1.0; self.severity = Severity.S4
            else:
                # Fallback intelligent pour ANALYZE, REVIEW, RECALC_TRAJECTORY
                self.confidence = 0.85; self.severity = Severity.S2
