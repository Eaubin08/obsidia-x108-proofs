    ...
    getPerformanceInsights: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.sessionTransactions.length < 10) {
          return {
            summary: `En attente de donn̩es (${input.totalSessionCount}/10 transactions de la session)`,
            trends: [],
            recommendations: [],
            metrics: {
              accuracyRate: 0,
    ...
    analyzeTrends: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.recentTransactions.length < 5) {
          return ["Pas assez de donn̩es pour d̩tecter des tendances"];
        }
    ...
    generateReport: publicProcedure
      ...
      .mutation(async ({ input }) => {
        if (input.sessionTransactions.length === 0) {
          return "Aucune transaction dans la session pour g̩n̩rer un rapport";
        }
