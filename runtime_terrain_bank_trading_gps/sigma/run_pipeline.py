...
def main():
    ...
    try:
        ...
        # P3T9B1_HOLD_REVIEW_RULE
        # Safety demotion only:
        # trading + REVIEW + ALLOW + irreversible trade execution review => HOLD.
        # Preserves BLOCK > HOLD > ALLOW and does not touch Guard/consensus.
        _p3t9b_irreversible_trade_review = (
            result_dict.get("domain") == "trading"
            and result_dict.get("x108_gate") == "ALLOW"
            and result_dict.get("market_verdict") == "REVIEW"
            and result_dict.get("irreversible") is True
            and result_dict.get("action_type") == "trade"
            and result_dict.get("intent") == "trade_execution_review"
        )
        if _p3t9b_irreversible_trade_review:
            ...
            result_dict["reason_code"] = "IRREVERSIBLE_TRADE_REVIEW_REQUIRES_HOLD"
            result_dict["x108_reason"] = "IRREVERSIBLE_TRADE_REVIEW_REQUIRES_HOLD"
            ...
            if not isinstance(result_dict.get("metrics"), dict):
                result_dict["metrics"] = {}
            _p3t9b_action_meta = result_dict["metrics"].get("action_metadata")
            if not isinstance(_p3t9b_action_meta, dict):
                _p3t9b_action_meta = {}
                result_dict["metrics"]["action_metadata"] = _p3t9b_action_meta
            _p3t9b_action_meta["business_semantic_verdict"] = "HOLD_REVIEW"
            _p3t9b_action_meta["p3t9b_semantic_rule"] = "IRREVERSIBLE_TRADE_REVIEW_REQUIRES_HOLD"
