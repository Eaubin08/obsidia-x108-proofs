...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OCTAVE_COMPAT_SHIM: this hackathon-local copy only forces save() calls
% to MAT binary format so Octave can persist class/object values. GNSS
% acquisition, tracking, observation, frame decoding, and navigation logic
% are unchanged from upstream FGI-GSRx v2.1.3.
...
if(settings.sys.saveDataFile == true)
    save("-mat7-binary",settings.sys.dataFileOut,'settings','acqData','ephData');
end
...
if(settings.sys.saveDataFile == true)
    save("-mat7-binary",settings.sys.dataFileOut,'settings','acqData','ephData','trackData');
end
...
% Save results so far to file
if(settings.sys.saveDataFile == true)
    save("-mat7-binary",settings.sys.dataFileOut,'settings','acqData','ephData','trackData','obsData');
end
...
% OCTAVE_COMPAT_SHIM: persist final navigation products for audit artefacts.
if(settings.sys.saveDataFile == true)
    save("-mat7-binary",settings.sys.dataFileOut,'settings','acqData','ephData','trackData','obsData','satData','navData','statResults');
end
