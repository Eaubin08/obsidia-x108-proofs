...
class AgentVote(UniversalBase):
    ...
    def __init__(self, *args, **kwargs):
        # Ordre reel utilise par sigma/domains/bank_agents.py :
        # AgentVote(agent_id, domain, layer, claim, confidence, severity_hint, proposed_verdict=...)
        if len(args) >= 2 and isinstance(args[1], Domain):
            field_order = [
                "agent_id",
                "domain",
                "layer",
...
class DomainAggregate(UniversalBase):
    ...
    def __init__(self, *args, **kwargs):
        explicit_confidence = kwargs.get("confidence", None)
        ...
        try:
            current_conf = float(getattr(self, "confidence", 0.0))
        except Exception:
            current_conf = 0.0

        if self.agent_votes and (explicit_confidence is None or current_conf == 0.0):
            confs = []
            for av in self.agent_votes:
                try:
                    confs.append(float(getattr(av, "confidence", 0.0)))
                except Exception:
                    confs.append(0.0)
        ...
        obsidia_log(
            f"Aggregating {len(self.agent_votes)} votes for {self.domain} "
            f"(Final Conf: {self.confidence})"
        )
