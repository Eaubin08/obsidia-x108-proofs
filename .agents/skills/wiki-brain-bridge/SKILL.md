...
3. **Never index protected paths** without explicit per-path approval from the user. Protected paths are listed in `.Codex/context/PROTECTED_SCOPE.md`. Default exclude list (mandatory):
...
Required exclusion list:  see .Codex/skills/wiki-brain-bridge/SKILL.md (full list)
...
- Running `Codex install` / `npm install` / any install command for Graphify automatically.
