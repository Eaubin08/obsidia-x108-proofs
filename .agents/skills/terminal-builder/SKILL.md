...
- User wants to inspect via CLI rather than via Codex reads.

## When NOT to use

- The user wants Codex to **directly modify files** — use `sigma-surgeon` or another action skill.
...
5. Never instruct the user to "open the file in an editor". Use `code`, `notepad`, or a Codex edit instead.
