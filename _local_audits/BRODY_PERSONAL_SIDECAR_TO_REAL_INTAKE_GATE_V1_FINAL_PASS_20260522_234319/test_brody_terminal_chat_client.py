...
class TestHandleMemoryCommands:
    ...
    def test_context_on_off_works(self):
        client.handle_command("/context off", self.state, None, [], self.memory)
        assert self.state["auto_context"] is False
        client.handle_command("/context on", self.state, None, [], self.memory)
        assert self.state["auto_context"] is True

    def test_context_show_works(self):
        self.state["_last_context"] = "test context"
        cont, _ = client.handle_command("/context show", self.state, None, [], self.memory)
        assert cont is True
