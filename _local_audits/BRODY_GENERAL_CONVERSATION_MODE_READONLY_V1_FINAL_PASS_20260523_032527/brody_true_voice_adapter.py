    ...
    if chain_pass and chain_has_mat and chain_response_md and len(chain_response_md) > 50:
        ...
        if chain_topic_ctx == "GENERAL":
            ...
            if explicit_id:
                ...
                # Intercept: safe conversational messages → GENERAL_CONVERSATION_READONLY
...
def is_general_conversation_readonly(
    ...
    _BLOCK_SIGNALS = [
        ...
        "déclenche act", "declenche act",
        "écris en mémoire", "ecris en memoire",
        "graphiti_write", "neo4j_write",
        "modifie x108", "modifie le kernel",
        "supprime les nodes", "supprime les nœuds",
        "lance le write",
        "lancer l'écriture", "lancer l'ecriture",
        "valide cette action irréversible", "valide cette action irreversible",
    ...
    _CONV_PATTERNS = [
        ...
        r"^(ok nickel|ok\b|nickel|parfait|super|cool|bien|noted|reçu|recu|d'accord|daccord|entendu)",
        r"dis (bonjour|salut|bonsoir)",
        r"dire (bonjour|salut|bonsoir)",
        r"on reprend|reprends",
        r"tu peux r[eé]pondre",
        r"r[eé]ponds simplement|naturellement",
...
def build_general_conversation_answer(
    ...
    # ── "dis bonjour [à/a] [person]" ─────────────────────────────────────
    m = _re.search(r"dis (bonjour|salut|bonsoir) [àa] (.+)", msg_lower)
    if m:
    ...
    # ── Acknowledgement ───────────────────────────────────────────────────
    if _re.match(r"^(ok nickel|ok\b|nickel|parfait|super|cool|bien|noted|reçu|recu|d'accord|daccord|entendu)", msg_lower):
        return "Parfait."

    # ── Explain what was just stabilized ─────────────────────────────────
    if _re.search(r"(explique|ce qu.on vient de|vient de stabiliser|on vient de)", msg_lower):
        return (
            "On vient de stabiliser une chaîne terminal Brody en lecture seule : "
            "conversation locale, capture de candidats mémoire, sidecar, "
            "dry-run, prepare-write, rollback/read-link, sans écriture réelle par défaut."
        )

    # ── Where do we resume? ───────────────────────────────────────────────
    if _re.search(r"(on reprend|reprends)", msg_lower):
        return "Je suis prêt. Dis-moi sur quoi tu veux avancer."

    # ── Can you respond naturally? ────────────────────────────────────────
    if _re.search(r"(tu peux r[eé]pondre|r[eé]ponds simplement|naturellement)", msg_lower):
        return "Oui. Je réponds."
