...
# ============================================================================
# F32-A ? READ_ONLY bound work-unit rehydration
# ============================================================================

def _f32a_rehydrate(env, mid):
    return M.rehydrate_bound_work_unit(
        mission_id=mid,
        mission_store_dir=env["stores"]["missions"],
        hold_store_dir=env["stores"]["holds"],
    )
...
def _f32a_binding_revision(env, mid):
    d = env["stores"]["missions"] / mid / "revisions"
    for path in sorted(d.glob("*.json")):
        rec = json.loads(path.read_text(encoding="utf-8"))
        if rec.get("event") == M.E_WORKTREE_CREATED:
            return path, rec
    raise AssertionError("WORKTREE_CREATED revision not found")


def _f32a_rewrite_binding(env, mid, mutate):
    path, rec = _f32a_binding_revision(env, mid)
    mutate(rec["evidence_refs"])
    rec["mission_revision_record_hash"] = M._record_hash(rec, M._REVISION_BOUND_FIELDS)
    path.write_text(json.dumps(rec, indent=2, sort_keys=True), encoding="utf-8")


def test_F32A_rehydrate_valid_bound_work_unit_exact_fields(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATED, r
    assert r["reason"] is None
    assert r["current_state"] == M.S_WORKTREE_BOUND

    wu = r["work_unit"]
    assert isinstance(wu, WU.IsolatedWorkUnit)
    assert wu.work_unit_id == env["wu"].work_unit_id
    assert wu.repo_root == str(env["main"].resolve())
    assert wu.main_worktree_path == str(env["main"].resolve())
    assert wu.base_sha == env["base_sha"]
    assert wu.branch_name == "bmbr"
    assert wu.worktree_path == str(env["wt_path"])
    assert wu.created_by_this_component is env["wu"].created_by_this_component


def test_F32A_rehydrate_missing_worktree_binding_rejected(env):
    mid = _genesis(env)["mission_id"]

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"] == "WORKTREE_BINDING_EVIDENCE_MISSING"
    assert r["work_unit"] is None


def test_F32A_duplicate_worktree_binding_fails_closed_at_projection(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)
    _, binding = _f32a_binding_revision(env, mid)

    _raw_append(
        env,
        mid,
        event=M.E_WORKTREE_CREATED,
        from_state=M.S_CREATED,
        to_state=M.S_WORKTREE_BOUND,
        evidence_refs=dict(binding["evidence_refs"]),
    )

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"].startswith("MISSION_NOT_PROJECTABLE:")
    assert r["work_unit"] is None


def test_F32A_binding_evidence_branch_mismatch_rejected(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)

    _f32a_rewrite_binding(
        env, mid,
        lambda ev: ev.__setitem__("verified_branch", "wrong-branch"),
    )

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"] == "WORKTREE_BINDING_EVIDENCE_BRANCH_MISMATCH"


def test_F32A_binding_evidence_path_mismatch_rejected(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)

    _f32a_rewrite_binding(
        env, mid,
        lambda ev: ev.__setitem__("worktree_path", str(env["main"].resolve())),
    )

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"] == "WORKTREE_BINDING_EVIDENCE_PATH_MISMATCH"


def test_F32A_created_by_this_component_must_remain_bool(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)

    _f32a_rewrite_binding(
        env, mid,
        lambda ev: ev.__setitem__("created_by_this_component", 1),
    )

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"] == "WORKTREE_BINDING_CREATED_BY_INVALID"


def test_F32A_worktree_bound_dirty_rejected(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)
    (env["wt_path"] / _TARGET_REL).write_bytes(b"LOCAL-DRIFT\n")

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"] == "WORKTREE_DRIFT:DIRTY"


def test_F32A_action_prepared_dirty_rejected(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)
    prep = _prepare(env, mid)
    assert prep["status"] == M.STATUS_ACTION_PREPARED, prep
    (env["wt_path"] / _TARGET_REL).write_bytes(b"LOCAL-DRIFT-AFTER-PREPARE\n")
...
def test_F32A_action_executed_kept_dirty_is_accepted(env):
    mid, prep, ex = _drive_mission_to_kept(env, objective="f32a-kept-dirty")
    assert prep["status"] == M.STATUS_ACTION_PREPARED, prep
    assert ex["status"] == M.STATUS_ACTION_RECORDED, ex
    assert _proj(env, mid)["current_state"] == M.S_ACTION_EXECUTED_KEPT
    assert _git(env["wt_path"], "status", "--porcelain") != ""
    ...
    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATED, r
    assert r["current_state"] == M.S_ACTION_EXECUTED_KEPT
    assert r["work_unit"].work_unit_id == env["wu"].work_unit_id


def test_F32A_held_from_worktree_bound_clean_is_accepted(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)
    h = _open_hold(env, mid)
    assert h["status"] == M.STATUS_HOLD_OPENED, h
    assert _proj(env, mid)["current_state"] == M.S_HELD

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATED, r
    assert r["current_state"] == M.S_HELD
    assert r["work_unit"].work_unit_id == env["wu"].work_unit_id


def test_F32A_head_drift_rejected(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)
    _git(env["wt_path"], "commit", "--allow-empty", "-m", "f32a-head-drift")
...
def test_F32A_repository_identity_drift_rejected(env):
    mid = _genesis(env)["mission_id"]
    _bind(env, mid)
    _git(env["main"], "worktree", "remove", "--force", str(env["wt_path"]))

    r = _f32a_rehydrate(env, mid)

    assert r["status"] == M.STATUS_WORK_UNIT_REHYDRATE_REJECTED
    assert r["reason"] == "MISSION_WORKTREE_BINDING_DRIFT:REPO_IDENTITY"
