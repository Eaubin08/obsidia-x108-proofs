...
class TestDedupSemantics:
    ...
    def test_replay_returns_same_immutable_source_facts(self, synthetic_repo, tmp_path):
        """F29 : un rejeu d'inscription expose les MEMES faits source immuables
        que la premiere inscription. Sans cela ALREADY_REGISTERED est accepte
        syntaxiquement par la preparation gouvernee mais reste inutilisable."""
        ledger_dir = tmp_path / "ledger"
        r1 = L.register_git_blob_source(
            "candidate", "src/module.py", repo_root=synthetic_repo, ledger_dir=ledger_dir,
        )
        ...
        assert r1["status"] == "DISCOVERED"
        assert r2["status"] == "ALREADY_REGISTERED"
        for field in ("ledger_entry_id", "source_git_commit_sha",
                      "source_git_blob_sha", "source_content_sha256"):
            assert field in r2, f"champ absent du rejeu : {field}"
            assert r2[field] == r1[field], f"divergence sur {field}"

    def test_replay_metadata_matches_persisted_entry(self, synthetic_repo, tmp_path):
        """Le rejeu DECRIT l'entree persistee ; il ne resynthetise rien."""
        ledger_dir = tmp_path / "ledger"
        L.register_git_blob_source(
            "candidate", "src/module.py", repo_root=synthetic_repo, ledger_dir=ledger_dir,
        )
        r2 = L.register_git_blob_source(
            "candidate", "src/module.py", repo_root=synthetic_repo, ledger_dir=ledger_dir,
        ...
        entries = L._load_entries(ledger_dir)
        assert len(entries) == 1, "le rejeu ne doit creer aucune entree supplementaire"
        persisted = entries[0]
        assert r2["ledger_entry_id"] == persisted["ledger_entry_id"]
        assert r2["source_git_commit_sha"] == persisted["source_git_commit_sha"]
        assert r2["source_git_blob_sha"] == persisted["source_git_blob_sha"]
        assert r2["source_content_sha256"] == persisted["source_content_sha256"]

    def test_replay_result_accepted_by_governed_preparation_check(self, synthetic_repo, tmp_path):
        """Le rejeu franchit le controle exact qui rejetait la preparation."""
        ...
        assert r2["status"] in ("DISCOVERED", "ALREADY_REGISTERED")
        assert _DRV._is_full_sha256(r2.get("source_content_sha256"))

    def test_replay_does_not_alias_a_different_source(self, synthetic_repo, tmp_path):
        """Une identite source differente ne doit JAMAIS aliaser l'entree precedente."""
        ledger_dir = tmp_path / "ledger"
        r1 = L.register_git_blob_source(
            "candidate", "src/module.py", repo_root=synthetic_repo, ledger_dir=ledger_dir,
        )
        r_other = L.register_git_blob_source(
            "candidate", "src/module.py", target_path="periphery/other_target.py",
            repo_root=synthetic_repo, ledger_dir=ledger_dir,
        )
        assert r_other["ledger_entry_id"] != r1["ledger_entry_id"]
        assert r_other["status"] == "DISCOVERED"

    def test_invalid_source_remains_fail_closed(self, synthetic_repo, tmp_path):
        """Une source invalide reste rejetee, avant toute logique de rejeu."""
        ledger_dir = tmp_path / "ledger"
        bad = L.register_git_blob_source(
            "deadbeef" * 5, "src/module.py",
            repo_root=synthetic_repo, ledger_dir=ledger_dir,
        )
        assert bad["status"] == "REJECTED"
        assert "source_content_sha256" not in bad
