"""
Tests SCRAPING_READONLY_STUB_V0
Vérifie : import, déterminisme, flags interdits false, authority, network, ACT, write,
          quarantine_policy, web_scrape_allowed.
SCRAP_ACTIVE=NO | NETWORK=NO | AUTHORITY=NONE | DECISION_AUTHORITY=KX108_ONLY
"""
import pytest
...
def test_import_ok():
    ...
    result = build_scraping_readonly_stub()
...
def test_output_deterministic_same_domain():
    ...
    r1 = build_scraping_readonly_stub(domain="test_domain")
    r2 = build_scraping_readonly_stub(domain="test_domain")
    assert r1 == r2


def test_output_deterministic_default_domain():
    """Appel sans argument : déterministe sur deux invocations."""
    r1 = build_scraping_readonly_stub()
    r2 = build_scraping_readonly_stub()
    assert r1 == r2


def test_stub_id_stable():
    """scraping_stub_id est stable entre les appels pour le même domaine."""
    r1 = build_scraping_readonly_stub(domain="cic_readonly_pack")
    r2 = build_scraping_readonly_stub(domain="cic_readonly_pack")
    assert r1["scraping_stub_id"] == r2["scraping_stub_id"]


def test_stub_id_differs_by_domain():
    """scraping_stub_id diffère si le domaine change."""
    r1 = build_scraping_readonly_stub(domain="domain_a")
    r2 = build_scraping_readonly_stub(domain="domain_b")
    assert r1["scraping_stub_id"] != r2["scraping_stub_id"]
...
def test_scraping_active_false():
    result = build_scraping_readonly_stub()
    assert result["scraping_active"] is False


def test_web_scrape_allowed_false():
    result = build_scraping_readonly_stub()
    assert result["web_scrape_allowed"] is False


def test_quarantine_policy_web_scrape_quarantined():
    """La politique de quarantaine web_scrape doit être déclarée immuable."""
    result = build_scraping_readonly_stub()
    assert result["quarantine_policy"] == "WEB_SCRAPE_QUARANTINED"


def test_ingestion_auto_false():
    result = build_scraping_readonly_stub()
    assert result["ingestion_auto"] is False
...
def test_authority_none():
    """authority doit être 'NONE' — Scraping n'est pas souverain."""
    result = build_scraping_readonly_stub()
...
# ---------------------------------------------------------------------------
# Aucun URL fetch dans les valeurs
# ---------------------------------------------------------------------------

def test_no_url_in_output():
    """Aucune valeur de type URL (http/https) dans le résultat."""
    result = build_scraping_readonly_stub()
    for v in result.values():
        if isinstance(v, str):
            assert not v.startswith("http://"), f"URL trouvée : {v}"
            assert not v.startswith("https://"), f"URL trouvée : {v}"
...
# ---------------------------------------------------------------------------
# Aucun appel réseau monkeypatché
# ---------------------------------------------------------------------------

def test_no_network_call_under_monkeypatch(monkeypatch):
    """
    ...
    def _blocked(*args, **kwargs):
        raise RuntimeError("Appel réseau interdit dans Scraping readonly stub")

    monkeypatch.setattr(socket, "getaddrinfo", _blocked)
    result = build_scraping_readonly_stub(domain="network_test")
    assert result["network"] is False
    assert result["scraping_active"] is False
